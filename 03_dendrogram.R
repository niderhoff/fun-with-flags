...
print(paste(
    "Generating dendrogram for", capwords(current_measure), "measure", sep = " ")
)
...
#    col = brewer.pal(n_cluster, "Paired")
    col = rep(c("tomato","slateblue"), n_cluster/2)
)
...
path = paste("figures/", current_measure, "_dendrogram.pdf", sep="")
pdf(path, width = 44, paper = "a4r")
plot(dendrogram, ylab = paste(capwords(current_measure),"Similarty Coefficient",sep =" "),
     cex.lab = 1.5,
     cex.axis = 1.5)
